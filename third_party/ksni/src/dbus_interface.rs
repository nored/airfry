...
impl<T: Tray> DbusMenu<T> {
    ...
    async fn about_to_show(&self, id: i32) -> zbus::fdo::Result<bool> {
        ...
        if service.get_menu_item(id, &[]).is_none() {
            ...
            // AIRFRY PATCH: notify the tray that its menu is opening so it can
            // lazily scan on-open (see Tray::about_to_show). Lightweight by
            // contract; the app hands off to a worker thread.
            service.tray.about_to_show();
            Ok(false)
        }
    }
