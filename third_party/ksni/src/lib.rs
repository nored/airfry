...
pub trait Tray: Sized + Send + 'static {
    ...
    /// The menu is about to be shown (the DBusMenu host called `AboutToShow`).
    ///
    /// AIRFRY PATCH: stock ksni answers `AboutToShow` internally and never tells
    /// the tray. We surface it so the app can lazily kick a device scan exactly
    /// when the user opens the tray menu (and nothing in the background), matching
    /// the patched-`AboutToShow` behaviour the Go build relied on. Keep this
    /// handler lightweight — hand work off to a worker thread and call
    /// [`crate::Handle::update`] when results arrive.
